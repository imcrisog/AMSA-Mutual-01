...
export default function FutbolNuevoPage() {
  ...
  return (
    <div className="flex flex-1 flex-col bg-zinc-50 px-6 py-16 dark:bg-black">
      <div className="mx-auto w-full max-w-xl rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-black">
        <h1 className="text-3xl font-extrabold tracking-tight">Nuevo torneo (Fútbol)</h1>
        <p className="mt-2 text-zinc-600 dark:text-zinc-400">
          Elegí un nombre para identificar el borrador.
        </p>
        ...
        <input
          ...
          placeholder="Ej: Champions amigos"
          value={name}
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          ...
          <button
            className="rounded-xl bg-gradient-to-r from-amber-500 to-yellow-500 px-4 py-3 text-sm font-extrabold tracking-wide text-black shadow-sm disabled:opacity-50"
            disabled={!canContinue || loading}
            onClick={async () => {
              try {
                const meta = await apiCreateTournament({ name, sport: "futbol" });
