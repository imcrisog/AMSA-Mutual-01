...
type FutbolStep = "equipos" | "formato" | "sorteo" | "resumen";
  ...
  return (
    <span
      className={
        ...
        (props.active ? "text-amber-700 dark:text-amber-300" : "text-zinc-500 dark:text-white/50")
      }
    >
      {props.children}
    </span>
...
export default function FutbolShell(props: {
  title: string;
  subtitle?: string;
  step: FutbolStep;
  children: React.ReactNode;
}) {
  const is = (s: FutbolStep) => props.step === s;

  return (
    <div className="relative flex flex-1 flex-col overflow-hidden bg-gradient-to-b from-sky-50 via-white to-white px-6 py-14 text-zinc-900 dark:from-slate-950 dark:via-blue-950 dark:to-black dark:text-white">
      {/* ambient lights */}
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute -bottom-40 right-[-120px] h-[520px] w-[520px] rounded-full bg-amber-400/15 blur-3xl dark:bg-amber-400/10" />
      </div>
      ...
      <header className="relative mx-auto w-full max-w-6xl">
        ...
        <div className="mt-10">
          <div className="inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.3em] text-amber-800 shadow-sm dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-200">
            <span className="text-sm">🏆</span>
            Modo torneo
          </div>

          <h1 className="mt-4 text-3xl font-extrabold tracking-tight sm:text-4xl">
            <span className="bg-gradient-to-r from-slate-900 via-slate-900 to-slate-900 bg-clip-text text-transparent dark:from-white dark:via-sky-200 dark:to-amber-200">
              {props.title}
            </span>
          </h1>
          {props.subtitle && (
