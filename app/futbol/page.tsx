...
import { apiGetDraft, apiGetTeams, apiSetTeams, apiListTournaments } from "@/lib/tournamentsApi";
...
export default function FutbolPage() {
  ...
  // Load teams for active tournament (IndexedDB) on first render.
  // Keep local-only fallback working if there is no active tournament id.
  useEffect(() => {
    if (loading) return;
    let cancelled = false;
    ...
    Promise.all([apiGetTeams(tournamentId), apiGetDraft(tournamentId)])
      .then(([t, d]) => {
        ...
        Promise.resolve().then(() => {
          ...
          // If tournament already has format + draw saved, jump to step 4/4.
          if (d?.format && (d.cupRounds || d.leagueMatches || d.groups)) {
            router.replace("/futbol/resumen");
            return;
          }

          // If format is chosen but draw is not completed, go to draw step.
          if (d?.format) {
            router.replace("/futbol/sorteo");
            return;
          }
    ...
    // best-effort: load tournament name for subtitle.
    apiListTournaments()
      .then((list) => {
  ...
  const canAddTeam = useMemo(
    () => teamName.trim().length > 0 && players.length > 0,
  ...
  function finishTeams() {
    ...
    apiSetTeams(tournamentId, teams)
      .then(() => router.push("/futbol/formato"))
      .catch((err) => {
  ...
  return (
    <FutbolShell
      step="equipos"
      title={tournamentName ? `Torneo: ${tournamentName}` : "Torneo de fútbol"}
      subtitle={
        tournamentName
          ? "Armá tus equipos, elegí el formato y cargá resultados."
          : "Armá tus equipos, elegí el formato y cargá resultados. Estilo noche de copa."
      }
    >
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        >
          ...
          <p className="mt-2 text-xs text-zinc-600 dark:text-zinc-400">
            Nota: para añadir un equipo se requiere al menos 1 jugador.
          </p>
        </FutbolCard>
